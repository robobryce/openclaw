...
import { t as backupCreateCommand } from "./backup-GZ4LtO0J.js";
import { n as buildMigrationReportDir, t as buildMigrationContext } from "./context-CFZZUiLc.js";
import { i as writeApplyResult, n as assertConflictFreePlan, t as assertApplySucceeded } from "./output-C6W3oGe0.js";
...
function reconcileInteractiveMigrationSkillToggleValues(selectedValues, activatedValue, selectableValues) {
	if (activatedValue === "__openclaw_migrate_skip_for_now__") return selectedValues.includes("__openclaw_migrate_skip_for_now__") ? [MIGRATION_SKILL_SELECTION_SKIP] : [];
	if (activatedValue === "__openclaw_migrate_toggle_all_on__") return [MIGRATION_SKILL_SELECTION_TOGGLE_ALL_ON, ...selectableValues];
	if (activatedValue === "__openclaw_migrate_toggle_all_off__") return [MIGRATION_SKILL_SELECTION_TOGGLE_ALL_OFF];
...
function reconcileInteractiveMigrationShortcutValues(previousValues, selectedValues, selectableValues, key) {
	...
	if (key === "a" && !previousValues.includes("__openclaw_migrate_skip_for_now__") && previousSelectable.length === selectableValues.length) return [MIGRATION_SKILL_SELECTION_TOGGLE_ALL_OFF];
	const selectedSelectable = selectedValues.filter((value) => selectableValues.includes(value));
	if (selectedSelectable.length === selectableValues.length) return [MIGRATION_SKILL_SELECTION_TOGGLE_ALL_ON, ...selectableValues];
...
//#endregion
export { MIGRATION_SKILL_SELECTION_TOGGLE_ALL_ON as a, formatMigrationSkillSelectionHint as c, getMigrationSkillSelectionValue as d, getSelectableMigrationSkillItems as f, resolveInteractiveMigrationSkillSelection as h, MIGRATION_SKILL_SELECTION_TOGGLE_ALL_OFF as i, formatMigrationSkillSelectionLabel as l, reconcileInteractiveMigrationSkillToggleValues as m, runMigrationApply as n, applyMigrationSelectedSkillItemIds as o, reconcileInteractiveMigrationShortcutValues as p, MIGRATION_SKILL_SELECTION_SKIP as r, applyMigrationSkillSelection as s, createPreMigrationBackup as t, getDefaultMigrationSkillSelectionValues as u };

...
import { r as createConfigIO } from "./io-qSKtb3D6.js";
...
import { l as formatPluginCompatibilityNotice, r as buildPluginCompatibilitySnapshotNotices } from "./status-C06l7B7p.js";
import { n as commitConfigWriteWithPendingPluginInstalls } from "./plugins-install-record-commit--ez-uaLQ.js";
import { t as resolveSetupSecretInputString } from "./setup.secret-input-Cez3SCGD.js";
import { r as runSetupMigrationImport, t as detectSetupMigrationSources } from "./setup.migration-import-BIcfXGcY.js";
...
function loadAuthChoiceModule() {
	authChoiceModulePromise ??= import("./auth-choice-DYigaa6s.js");
	return authChoiceModulePromise;
}
function loadConfigLoggingModule() {
	configLoggingModulePromise ??= import("./logging-CFE7Ml-o.js");
	return configLoggingModulePromise;
}
function loadModelPickerModule() {
	modelPickerModulePromise ??= import("./model-picker-Ce0avIiu.js");
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const [{ resolveManifestProviderAuthChoice }, { resolvePluginSetupProvider }] = await Promise.all([import("./provider-auth-choices-BGgCKJl6.js"), import("./setup-registry-BAm1INWe.js")]);
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-BMG3s9pe.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-CObH5VQp.js");
		const { applySkipBootstrapConfig } = await import("./onboard-config-DXJCedUA.js");
	...
	const { applyLocalSetupWorkspaceConfig, applySkipBootstrapConfig } = await import("./onboard-config-DXJCedUA.js");
	...
	if (authChoiceFromPrompt) {
		...
		({promptAuthChoiceGrouped} = await import("./auth-choice-prompt-DlK_R8Is.js"));
	...
	while (true) {
		...
		if (authChoice === "custom-api-key") {
			const { promptCustomApiConfig } = await import("./onboard-custom-DKBGGFN7.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-Bc23ctpp.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-j-rWRyAq.js");
		const { setupChannels } = await import("./onboard-channels-DY78RD6V.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-vHn9GGs5.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-CBYPSht7.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupOfficialPluginInstalls } = await import("./setup.official-plugins-D51sxwfx.js");
		...
		const { setupPluginConfig } = await import("./setup.plugin-config-CqYFQuC3.js");
	...
	const { setupInternalHooks } = await import("./onboard-hooks-DuZqsg6k.js");
	...
	nextConfig = await writeWizardConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-KiIqQNm-.js");

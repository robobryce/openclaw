...
async function refreshLatestUpdateRestartSentinelIfPresent() {
	...
	return await (await import("./server-restart-sentinel-C7uXdmUp.js")).refreshLatestUpdateRestartSentinel();
}
...
async function startGatewaySidecars(params) {
	...
	if (params.cfg.acp?.enabled) (async () => {
		...
		const [{ getAcpSessionManager }, { ACP_SESSION_IDENTITY_RENDERER_VERSION }] = await Promise.all([import("./manager-BoS_qd4i.js"), import("./session-identifiers-hE-zRO3S.js")]);
	...
	schedulePostReadySidecarTask({
		...
		run: async () => {
			...
			setTimeout(() => {
				import("./server-restart-sentinel-C7uXdmUp.js").then(({ scheduleRestartSentinelWake }) => scheduleRestartSentinelWake({ deps: params.deps })).catch((err) => {
	...
	schedulePostReadySidecarTask({
		...
		log: params.log,
		run: async () => {
			const { scheduleSubagentOrphanRecovery } = await import("./subagent-registry-Cg1ynE9O.js");

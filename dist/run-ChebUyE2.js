...
import { n as acquireGatewayLock, t as GatewayLockError } from "./gateway-lock-CLrbXhwA.js";
...
async function resolveGatewayPasswordOption(opts) {
	...
	if (file) {
		const { readSecretFromFile } = await import("./secret-file-D7-Odc9z.js");
...
async function maybeLogPendingControlUiBuild(cfg) {
	...
	const { resolveControlUiRootSync } = await import("./control-ui-assets-DURV9nUH.js");
...
async function runGatewayCommand(opts) {
	...
	const { startGatewayServer } = await startupTrace.measure("cli.server-import", () => withProgress({
		...
		indeterminate: true
	}, async () => import("./server-BPpgz-SF.js")));
	setConsoleTimestampPrefix(true);
	if (devMode) {
		const { ensureDevGatewayConfig } = await import("./dev-Dzary4Xt.js");
	...
	if (opts.force) try {
		const { forceFreePortAndWait, waitForPortBindable } = await import("./ports-Ab9Nwdgd.js");
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			const { maybeExplainGatewayServiceStop } = await import("./shared-C_ArCtWb.js");

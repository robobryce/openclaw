...
import { A as applyConfigOverrides, c as promoteConfigSnapshotToLastKnownGood, i as getRuntimeConfig, u as readConfigFileSnapshot, v as registerConfigWriteListener } from "./io-qSKtb3D6.js";
...
import { m as setPreRestartDeferralCheck, p as setGatewaySigusr1RestartPolicy } from "./restart-D508Yjx_.js";
import { t as applyPluginAutoEnable } from "./plugin-auto-enable-CTA4EOML.js";
import { b as resolveActivePluginHttpRouteRegistry, d as pinActivePluginChannelRegistry, f as pinActivePluginHttpRouteRegistry, h as releasePinnedPluginHttpRouteRegistry, m as releasePinnedPluginChannelRegistry } from "./runtime-DPCm5dpS.js";
...
import { _ as stopDiagnosticHeartbeat, g as startDiagnosticHeartbeat } from "./diagnostic-D8XF5_t3.js";
...
import { i as getActiveSecretsRuntimeSnapshot, n as clearSecretsRuntimeSnapshot } from "./runtime-sVYlykXu.js";
...
import { n as logRejectedLargePayload } from "./diagnostic-payload-85AC-PW4.js";
import { n as sendGatewayAuthFailure, o as setDefaultSecurityHeaders } from "./http-common-BySbW3n7.js";
import { i as MAX_BUFFERED_BYTES, o as MAX_PREAUTH_PAYLOAD_BYTES } from "./server-constants-BH4FZ1-t.js";
import { t as GatewayLockError } from "./gateway-lock-CLrbXhwA.js";
import { c as resolveControlUiRootSync, n as isPackageProvenControlUiRootSync, s as resolveControlUiRootOverrideSync, t as ensureControlUiAssetsBuilt } from "./control-ui-assets-D2PvKXSm.js";
...
import { n as ensureControlUiAllowedOriginsForNonLoopbackBind } from "./gateway-control-ui-origins-BjmHGUi2.js";
import { n as resolveAssistantIdentity } from "./assistant-identity-CcfqJLy9.js";
import { i as getRequiredSharedGatewaySessionGeneration, r as enforceSharedGatewaySessionGenerationForConfigWrite } from "./server-shared-auth-generation-D9tyKjMB.js";
import { a as createToolEventRecipientRegistry, n as createChatRunState } from "./server-chat-state-BFg-mmjU.js";
import { a as incrementPresenceVersion, i as getPresenceVersion, n as getHealthCache, o as refreshGatewayHealthSnapshot, r as getHealthVersion } from "./health-state-17opwBBl.js";
import { i as setFallbackGatewayContextResolver } from "./server-plugins-CQSaV0VL.js";
import { n as applyGatewayLaneConcurrency, t as resolveHookClientIpConfig } from "./hook-client-ip-config-E_rckS3U.js";
import { t as GATEWAY_EVENTS } from "./server-methods-list-zvjSqJsu.js";
import { t as STARTUP_UNAVAILABLE_GATEWAY_METHODS } from "./server-startup-unavailable-methods-DRmPDWAC.js";
import { t as resolveSharedGatewaySessionGeneration } from "./ws-shared-generation-BicOnFbH.js";
import { n as isProtectedPluginRoutePathFromContext, r as resolvePluginRoutePathContext } from "./route-match-BTuZrTMb.js";
import { n as shouldEnforceGatewayAuthForPluginPath } from "./route-auth-CMiAHvow.js";
import { t as findMatchingPluginNodeCapabilityRoute } from "./route-capability-Bjqk111x.js";
...
function createLazyGatewayCronState(params) {
	...
	const load = async () => {
		...
		loading ??= import("./server-cron-BYAaSvhz.js").then(({ buildGatewayCronService }) => {
			loaded = {
...
function getControlUiModule() {
	controlUiModulePromise ??= import("./control-ui-2kEAMjEM.js");
	return controlUiModulePromise;
}
function getEmbeddingsHttpModule() {
	embeddingsHttpModulePromise ??= import("./embeddings-http-oIduG2kI.js");
	return embeddingsHttpModulePromise;
}
function getManagedImageAttachmentsModule() {
	managedImageAttachmentsModulePromise ??= import("./managed-image-attachments-DT-xpUwK.js");
	return managedImageAttachmentsModulePromise;
}
function getModelsHttpModule() {
	modelsHttpModulePromise ??= import("./models-http-D8hqp3h3.js");
	return modelsHttpModulePromise;
}
function getOpenAiHttpModule() {
	openAiHttpModulePromise ??= import("./openai-http-B2LKu9Qx.js");
	return openAiHttpModulePromise;
}
function getOpenResponsesHttpModule() {
	openResponsesHttpModulePromise ??= import("./openresponses-http-DASlN0mZ.js");
	return openResponsesHttpModulePromise;
}
function getSessionHistoryHttpModule() {
	sessionHistoryHttpModulePromise ??= import("./sessions-history-http-DwQM3utn.js");
	return sessionHistoryHttpModulePromise;
}
function getSessionKillHttpModule() {
	sessionKillHttpModulePromise ??= import("./session-kill-http-DIc1Rzr9.js");
	return sessionKillHttpModulePromise;
}
function getToolsInvokeHttpModule() {
	toolsInvokeHttpModulePromise ??= import("./tools-invoke-http-Cn8-ynPp.js");
	return toolsInvokeHttpModulePromise;
}
function getPluginNodeCapabilityAuthModule() {
	pluginNodeCapabilityAuthModulePromise ??= import("./plugin-node-capability-auth-DHmJbFVj.js");
	return pluginNodeCapabilityAuthModulePromise;
}
function getHttpAuthUtilsModule() {
	httpAuthUtilsModulePromise ??= import("./http-auth-utils-ta0zJYtr.js");
	return httpAuthUtilsModulePromise;
}
function getPluginRouteRuntimeScopesModule() {
	pluginRouteRuntimeScopesModulePromise ??= import("./plugin-route-runtime-scopes-DSm33EwY.js");
	...
	try {
		...
		const handleHooksRequest = async (req, res) => {
			...
			if (!loadedHooksRequestHandler) {
				const { createGatewayHooksRequestHandler } = await import("./hooks-B8yLj6Us.js");
		...
		const handlePluginRequest = async (req, res, pathContext, dispatchContext) => {
			...
			if (!loadedPluginRequestHandler) {
				const { createGatewayPluginRequestHandler } = await import("./plugins-http-Dkawe8hl.js");
		...
		const handlePluginUpgrade = async (req, socket, head, pathContext, dispatchContext) => {
			...
			if (!loadedPluginUpgradeHandler) {
				const { createGatewayPluginUpgradeHandler } = await import("./plugins-http-Dkawe8hl.js");
...
async function __resetModelCatalogCacheForTest() {
	const { __resetModelCatalogCacheForTest: resetModelCatalogCacheForTest } = await import("./server-model-catalog-DOwzG9UL.js");
...
function loadGatewayStartupEarlyModule() {
	gatewayStartupEarlyModulePromise ??= import("./server-startup-early-D0AJuT3r.js");
	return gatewayStartupEarlyModulePromise;
}
function loadGatewayStartupPostAttachModule() {
	gatewayStartupPostAttachModulePromise ??= import("./server-startup-post-attach-uDjIfr8V.js");
...
function getChannelRuntime() {
	cachedChannelRuntimePromise ??= import("./runtime-channel-Y7j_TCbH.js").then(({ createRuntimeChannel }) => createRuntimeChannel());
...
const loadGatewayModelCatalog = async (...args) => {
	gatewayModelCatalogModulePromise ??= import("./server-model-catalog-DOwzG9UL.js");
...
async function stopTaskRegistryMaintenanceOnDemand() {
	const { stopTaskRegistryMaintenance } = await import("./task-registry.maintenance-ICoHPvyK.js");
...
const runDefaultSetupWizard = async (...args) => {
	const { runSetupWizard } = await import("./setup-DzWvSu_1.js");
	return runSetupWizard(...args);
};
async function startGatewayServer(port = 18789, opts = {}) {
	const { bootstrapGatewayNetworkRuntime } = await import("./server-network-runtime-DNGkc2tY.js");
	...
	const startupConfigModulePromise = import("./server-startup-config-CjN9VOWS.js");
	let startupPluginsModulePromise = null;
	const loadStartupPluginsModule = () => {
		startupPluginsModulePromise ??= import("./server-startup-plugins-D2edw8EV.js");
	...
	const runtimeConfig = await startupTrace.measure("runtime.config", async () => {
		const { resolveGatewayRuntimeConfig } = await import("./server-runtime-config-B5oCSafw.js");
	...
	const { createChannelManager } = await import("./server-channels-DvC3Ds67.js");
	...
	const { createGatewayNodeSessionRuntime } = await import("./server-node-session-runtime-B1PvX6zc.js");
	...
	try {
		...
		const [{ startGatewayEventSubscriptions }, gatewayRuntimeServices] = await Promise.all([import("./server-runtime-subscriptions-BgLtogvv.js"), import("./server-runtime-services-tN8Saa71.js")]);
		...
		const { createGatewayAuxHandlers } = await import("./server-aux-handlers-DiNAIPpR.js");
		...
		const reloadAttachedGatewayPlugins = async (params) => {
			...
			const [{ loadPluginLookUpTable }, { prepareGatewayPluginLoad }, { startPluginServices }] = await Promise.all([
				import("./plugin-lookup-table-BpWj-iVa.js"),
				import("./server-plugin-bootstrap-hABbaxPm.js"),
				import("./services-C17DPool.js")
			]);
		...
		const { createGatewayRequestContext } = await import("./server-request-context-0tbnt8jG.js");
		...
		if (!minimalTestGateway) {
			if (runtimePluginsLoaded && deferredConfiguredChannelPluginIds.length > 0) {
				const { reloadDeferredGatewayPlugins } = await import("./server-plugin-bootstrap-hABbaxPm.js");
		...
		const { attachGatewayWsHandlers } = await import("./server-ws-runtime-D62nvAcy.js");
		const { listPluginNodeCapabilities } = await import("./route-capability-D39m60_F.js");
		...
		activateScheduledServicesWhenReady();
		const { startManagedGatewayConfigReloader } = await import("./server-reload-handlers-CpkugiHj.js");

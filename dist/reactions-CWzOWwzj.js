...
function createBlueBubblesSendReceipt(params) {
	...
	if (results[0] && params.chatGuid) results[0].conversationId = params.chatGuid;
...
async function parseBlueBubblesMessageResponse(res, params = {}) {
	...
	try {
		messageId = extractBlueBubblesMessageId(JSON.parse(body));
	} catch {
		messageId = "ok";
	}
	return {
...
async function sendMessageBlueBubbles(to, text, opts = {}) {
	...
	return parseBlueBubblesMessageResponse(res, {
		chatGuid,
		...wantsReplyThread && opts.replyToMessageGuid ? { replyToMessageGuid: opts.replyToMessageGuid } : {}
	});
}
//#endregion
...
function sanitizeFilename(input, fallback) {
	return sanitizeUntrustedFileName(input ?? "", fallback).replace(/[\r\n"\\]/g, "_");
}

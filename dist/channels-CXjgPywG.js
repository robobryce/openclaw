...
import { t as resolveInstallableChannelPlugin } from "./channel-plugin-resolution-B1J_3lXw.js";
import { t as resolveCommandConfigWithSecrets } from "./command-config-resolution-DglbGjNZ.js";
import { n as resolveLogFile } from "./log-tail-D7HXZsOj.js";
import { t as parseLogLine } from "./parse-log-line-D8CI8E6o.js";
import { a as describeBinding, t as applyAgentBindings } from "./agents.bindings-DEMADJBO.js";
import { t as requireValidConfigFileSnapshot } from "./config-validation-CusiaUbC.js";
import { c as shouldUseWizard, o as formatChannelAccountLabel, s as requireValidConfig } from "./shared-OtNocdxo.js";
import { t as channelsListCommand } from "./list-C-pa2r0G.js";
import { n as formatGatewayChannelsStatusLines, t as channelsStatusCommand } from "./status-JDEkqCk9.js";
...
const channelSetupPluginInstallLoader = createLazyImportLoader(() => import("./plugin-install-DxGaNT3O.js"));
const onboardChannelsLoader = createLazyImportLoader(() => import("./onboard-channels-CNmAOa28.js"));
...
async function resolveCatalogChannelEntry(raw, cfg) {
	...
	if (!trimmed) return;
	return (cfg ? await import("./trusted-catalog-BwPtkHCg.js").then(({ listTrustedChannelPluginCatalogEntries }) => listTrustedChannelPluginCatalogEntries({
		cfg,
		workspaceDir: resolveAgentWorkspaceDir(cfg, resolveDefaultAgentId(cfg))
	})) : await import("./catalog-DHTKXODj.js").then(({ listChannelPluginCatalogEntries }) => listChannelPluginCatalogEntries({ excludeWorkspace: true }))).find((entry) => {
...
async function channelsAddCommand(opts, runtime = defaultRuntime, params) {
	...
	if (shouldUseWizard(params)) {
		const [{ buildAgentSummaries }, onboardChannels] = await Promise.all([import("./agents.config-DqDf97mf.js"), loadOnboardChannels()]);
	...
	if (!channel && catalogEntry) {
		...
		const { isCatalogChannelInstalled } = await import("./discovery-BRrehNiD.js");
...
async function channelsRemoveCommand(opts, runtime = defaultRuntime, params) {
	...
	const resolvedPluginState = Boolean(lookupChannel || channel) ? await (async () => {
		const { resolveInstallableChannelPlugin } = await import("./channel-plugin-resolution-DkKqGyWa.js");

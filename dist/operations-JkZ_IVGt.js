...
async function runGatewayLifecycle(operation) {
	const lifecycle = await import("./lifecycle-DEG8rQmH.js");
	if (operation === "start") {
...
async function loadOverviewForOperation(deps) {
	...
	const { loadCrestodianOverview } = await import("./overview-BmJ9ca7o.js");
...
async function formatOverviewForOperation(overview, deps) {
	...
	const { formatCrestodianOverview } = await import("./overview-BmJ9ca7o.js");
...
async function executeCrestodianOperation(operation, runtime, opts = {}) {
	...
	if (operation.kind === "setup") {
		...
		const applyDefaultModelPrimaryUpdate = setupModel.model ? (await import("./shared-Bg9qr5A-.js")).applyDefaultModelPrimaryUpdate : void 0;
	...
	if (operation.kind === "config-set") {
		...
		await (opts.deps?.runConfigSet ?? (async (setOpts) => {
			const { runConfigSet: importedRunConfigSet } = await import("./config-cli-BVhylegS.js");
	...
	if (operation.kind === "create-agent") {
		...
		const before = await readConfigFileSnapshot();
		const workspace = resolveUserPath(operation.workspace ?? process.cwd());
		await (opts.deps?.runAgentsAdd ?? (await import("./agents2.commands.add-CLqECsb5.js")).agentsAddCommand)({
	...
	if (operation.kind === "doctor") {
		...
		await (opts.deps?.runDoctor ?? (await import("./doctor-DNmAVEuk.js")).doctorCommand)(runtime, { nonInteractive: true });
	...
	if (operation.kind === "doctor-fix") {
		...
		const { readConfigFileSnapshot } = await import("./config/config.js");
		const before = await readConfigFileSnapshot();
		await (opts.deps?.runDoctor ?? (await import("./doctor-DNmAVEuk.js")).doctorCommand)(runtime, {
			nonInteractive: true,
			repair: true,
			yes: true
		});
	...
	if (operation.kind === "status") {
		...
		const { statusCommand } = await import("./status.command-DhYJTGTy.js");
	...
	if (operation.kind === "open-tui") {
		...
		const result = await (opts.deps?.runTui ?? (await import("./tui-FhRYXEs5.js")).runTui)({
			local: true,
			session,
			deliver: false,
			historyLimit: 200
	...
	if (operation.kind === "set-default-model") {
		...
		const { mutateConfigFile, readConfigFileSnapshot } = await loadConfigFileMutationHelpers();
		const before = await readConfigFileSnapshot();
		const { applyDefaultModelPrimaryUpdate } = await import("./shared-Bg9qr5A-.js");
		const result = await mutateConfigFile({
			base: "source",
			mutate: (cfg) => {

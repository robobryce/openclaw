...
import { r as createConfigIO } from "./io-BvGD_Bil.js";
...
import { l as formatPluginCompatibilityNotice, r as buildPluginCompatibilitySnapshotNotices } from "./status-SdUdAbPh.js";
import { n as commitConfigWriteWithPendingPluginInstalls } from "./plugins-install-record-commit-1YEayUS0.js";
import { t as resolveSetupSecretInputString } from "./setup.secret-input-Cx7t-hQD.js";
import { r as runSetupMigrationImport, t as detectSetupMigrationSources } from "./setup.migration-import-CCYIWXuf.js";
...
function loadAuthChoiceModule() {
	authChoiceModulePromise ??= import("./auth-choice-DhNnEjEb.js");
	return authChoiceModulePromise;
}
function loadConfigLoggingModule() {
	configLoggingModulePromise ??= import("./logging-DFFxdp84.js");
	return configLoggingModulePromise;
}
function loadModelPickerModule() {
	modelPickerModulePromise ??= import("./model-picker-CJbnL6D_.js");
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const [{ resolveManifestProviderAuthChoice }, { resolvePluginSetupProvider }] = await Promise.all([import("./provider-auth-choices-DSixoVHX.js"), import("./setup-registry-BAm1INWe.js")]);
...
async function runSetupWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-CEH-pqb8.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-SROuafLK.js");
		const { applySkipBootstrapConfig } = await import("./onboard-config-DBY8DCYf.js");
	...
	const { applyLocalSetupWorkspaceConfig, applySkipBootstrapConfig } = await import("./onboard-config-DBY8DCYf.js");
	...
	if (authChoiceFromPrompt) {
		...
		({promptAuthChoiceGrouped} = await import("./auth-choice-prompt-BkzVx-HI.js"));
	...
	while (true) {
		...
		if (authChoice === "custom-api-key") {
			const { promptCustomApiConfig } = await import("./onboard-custom-C5t8UfR-.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-BCh4--9j.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { setupChannels } = await import("./onboard-channels-CNmAOa28.js");
	...
	if (opts.skipSearch) await prompter.note("Skipping search setup.", "Search");
		...
		const { setupSearch } = await import("./onboard-search-AghmGT-7.js");
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-B_hDc4Gv.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupOfficialPluginInstalls } = await import("./setup.official-plugins-DcGRAKEC.js");
		...
		const { setupPluginConfig } = await import("./setup.plugin-config-Cn0uTbfH.js");
	...
	const { setupInternalHooks } = await import("./onboard-hooks-o34l6fGy.js");
	...
	nextConfig = await writeWizardConfigFile(nextConfig);
	const { finalizeSetupWizard } = await import("./setup.finalize-BYD4o7Ys.js");

...
async function planAllModelListSources(params) {
	...
	const { loadStaticManifestCatalogRowsForList, loadSupplementalManifestCatalogRowsForList } = await import("./list.manifest-catalog-lTw3tKCM.js");
	if (!params.providerFilter) {
		const { loadProviderIndexCatalogRowsForList } = await import("./list.provider-index-catalog-DZco-d9x.js");
		return createSourcePlan({
			kind: "registry",
	...
	const { hasProviderStaticCatalogForFilter } = await import("./list.provider-catalog-DAswyW-Z.js");

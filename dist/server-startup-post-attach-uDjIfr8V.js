...
import { t as GATEWAY_EVENT_UPDATE_AVAILABLE } from "./events-zvzlCB8S.js";
import { t as hasConfiguredInternalHooks } from "./configured-C1SEUklS.js";
...
function scheduleGatewayMemoryBackend(params) {
	...
	const start = () => {
		import("./server-startup-memory-D0Mr64Hh.js").then(({ startGatewayMemoryBackend }) => startGatewayMemoryBackend({
			cfg: params.cfg,
			log: params.log
		})).catch((err) => {
...
async function refreshLatestUpdateRestartSentinelIfPresent() {
	...
	return await (await import("./server-restart-sentinel-BpI-jCrH.js")).refreshLatestUpdateRestartSentinel();
}
...
async function prewarmConfiguredPrimaryModel(params) {
	...
	const [{ resolveAgentWorkspaceDir, resolveDefaultAgentDir, resolveDefaultAgentId }, { DEFAULT_MODEL, DEFAULT_PROVIDER }, { isCliProvider, resolveConfiguredModelRef }, { resolveEmbeddedAgentRuntime }] = await Promise.all([
		...
		import("./model-selection-BkmaTNZN.js"),
	...
	const { ensureOpenClawModelsJson } = await import("./models-config-BomFCN8Q.js");
...
async function startGatewaySidecars(params) {
	await measureStartup(params.startupTrace, "sidecars.gmail-watch", async () => {
		if (params.cfg.hooks?.enabled && params.cfg.hooks.gmail?.account) {
			const { startGmailWatcherWithLogs } = await import("./gmail-watcher-lifecycle-CnVkjZbl.js");
	...
	await measureStartup(params.startupTrace, "sidecars.gmail-model", async () => {
		if (params.cfg.hooks?.gmail?.model) {
			const [{ DEFAULT_MODEL, DEFAULT_PROVIDER }, { loadModelCatalog }, { getModelRefStatus, resolveConfiguredModelRef, resolveHooksGmailModel }] = await Promise.all([
				import("./defaults-C94FBkKL.js"),
				import("./model-catalog-Ce4_TzWS.js"),
				import("./model-selection-BkmaTNZN.js")
			]);
	...
	await measureStartup(params.startupTrace, "sidecars.internal-hooks", async () => {
		try {
			if (internalHooksConfigured) {
				const [{ setInternalHooksEnabled }, { loadInternalHooks }] = await Promise.all([import("./internal-hooks-qCbHNo6m.js"), import("./loader-D8NHkRjI.js")]);
	...
	await measureStartup(params.startupTrace, "sidecars.plugin-services", async () => {
		try {
			const { startPluginServices } = await import("./services-C17DPool.js");
	...
	if (params.cfg.acp?.enabled) (async () => {
		...
		const [{ getAcpSessionManager }, { ACP_SESSION_IDENTITY_RENDERER_VERSION }] = await Promise.all([import("./manager-DEpMs7dE.js"), import("./session-identifiers-BQjvzop0.js")]);
			...
			try {
				...
				for (const sessionsDir of sessionDirs) {
					...
					if (result.cleaned.length > 0) {
						const { markRestartAbortedMainSessionsFromLocks } = await import("./main-session-restart-recovery-LXZq1mDQ.js");
	...
	schedulePostReadySidecarTask({
		...
		run: async () => {
			...
			setTimeout(() => {
				import("./server-restart-sentinel-BpI-jCrH.js").then(({ scheduleRestartSentinelWake }) => scheduleRestartSentinelWake({ deps: params.deps })).catch((err) => {
	...
	schedulePostReadySidecarTask({
		...
		log: params.log,
		run: async () => {
			const { scheduleSubagentOrphanRecovery } = await import("./subagent-registry-aWwzjRbk.js");
	...
	schedulePostReadySidecarTask({
		...
		log: params.log,
		run: async () => {
			const { scheduleRestartAbortedMainSessionRecovery } = await import("./main-session-restart-recovery-LXZq1mDQ.js");
...
const defaultGatewayPostAttachRuntimeDeps = {
	...
	logGatewayStartup: async (params) => (await import("./server-startup-log-DX5pjBQm.js")).logGatewayStartup(params),
	refreshLatestUpdateRestartSentinel: refreshLatestUpdateRestartSentinelIfPresent,
	scheduleGatewayUpdateCheck: async (...args) => (await import("./update-startup-B287pSZX.js")).scheduleGatewayUpdateCheck(...args),
	startGatewaySidecars,
	startGatewayTailscaleExposure: async (...args) => (await import("./server-tailscale-fn608bKt.js")).startGatewayTailscaleExposure(...args)
};
